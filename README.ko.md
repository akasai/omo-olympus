...
[oh-my-opencode](https://github.com/code-yeongyu/oh-my-openagent)를 쓰고 계시죠. 에이전트가 병렬로 실행되고, 백그라운드 태스크가 날아다니고, 세션이 생겼다 사라집니다.

근데 그게 **보이지 않습니다.**

omo-olympus가 해결합니다. 7명의 에이전트. 7개의 그리스 신화 페르소나. 사이드바에서 실시간 대사가 흘러갑니다. Explorer가 코드를 뒤지고 있으면 — *"I smell a clue"*. Oracle이 깊이 생각 중이면 — *"The vision forms..."*. Momus가 당신의 계획을 찢고 있으면 — *"Don't shoot the messenger"*.

비활성 에이전트는 💤. 활성 에이전트는 말합니다. 신들이 일하는 걸 지켜보세요.

## 설치

LLM 에이전트(Claude Code, opencode, Cursor 등)에 아래 프롬프트를 붙여넣으세요:
...
opencode가 시작할 때 npm 패키지를 자동 설치합니다. 더 만질 건 없습니다.
...
```
task() → 서버가 에이전트 타입 감지 → 대기열 → TUI가 세션 이벤트에서 pop → 사이드바가 말한다
```

**서버 플러그인**이 `tool.execute.before`를 후킹합니다. `task()` 호출을 가로채고 `subagent_type`, `category`, description 키워드로 에이전트를 추론합니다.

**TUI 플러그인**이 `sidebar_content`에 렌더링합니다 (사이드바 최상단, order 50). 파일 기반 IPC로 자식 세션을 페르소나에 매핑하고, Solid.js 리액티브 시그널로 실시간 업데이트합니다.
...
|   | 기능 | 왜 필요한가 |
|:---:|---|---|
| 📂 | **접기/펼치기** | 헤더 클릭으로 토글. 접힌 상태에서 활성 에이전트 수 표시 |
| 🎨 | **테마 연동** | opencode 테마 색상 자동 반영. RGBA buffer → hex 변환 |
| ⚡ | **실시간** | Solid.js 리액티브 시그널. 에이전트 작업과 동시에 사이드바 업데이트 |
| 🔇 | **자동 수면** | 작업 완료 3초 후 💤로 복귀 |
...
그래서 그리스 신화에서 이름을 따오고, 역할에 맞는 성격을 입히고, 매번 바뀌는 대사를 넣었습니다. 사소한 것일 수도 있습니다. 하지만 Sisyphus가 바위를 밀어올리는 동안 Oracle이 안개 너머를 들여다보는 걸 한 번 보고 나면 — 조용한 사이드바로는 돌아가기 어렵습니다.
